...
import at.flauschigesalex.dsb.data.RoleData
...
class SupportTicket private constructor(val category: SupportCategory,
                                        val parent: GuildConfiguration,
                                        private val json: JsonManager,
): Comparable<SupportTicket>, Serializable() {
    ...
    companion object {
        ...
        operator fun invoke(category: SupportCategory, member: Member, channel: TextChannel): SupportTicket {
            val json = JsonManager(
                ...
                "ids.type" to category.id,
            )
            
            return SupportTicket(category, category.parent, json)
        }
        operator fun invoke(parent: GuildConfiguration, json: JsonManager): SupportTicket? {
            val typeId = json.getString("ids.type") ?: return null
            val type = parent.supportCategories.find { it.id == typeId } ?: run {
                logger.warn("Ticket could not be created since no type matches '$typeId'.")
                return null
            }
            
            return SupportTicket(type, parent, json)
        }
        
        val scheduler: ScheduledFuture<*> = Executors.newSingleThreadScheduledExecutor().scheduleWithFixedDelay({
            val managers = TicketManager.all()

            val result = runCatching { managers.flatMap { it.tickets }.forEach { ticket ->
                ...
                if (autoCloseDuration != null && state == State.OPEN) {
                    ...
                    if (expireIn.isBefore(now)) {
                        ticket.close(null, ticket.channel!!, null, false)
                    }
                    return@forEach
                }

                if (state == State.CLOSED) {
                    val archiveTimestamp = (ticket.closeData?.timestamp ?: ticket.latestInteraction)
                ...
                if (state == State.ARCHIVED) {
                    ...
                    if (now.isAfter(archiveTimestamp))
                        ticket.manager.removeTicket(ticket)
                    return@forEach
                }
            } }
            
            if (result.isFailure) logger.error("Scheduled task threw an exception:", result.exceptionOrNull())

            managers.forEach { it.saveAll(false) }
        }, 0, 1, TimeUnit.SECONDS)
    }
    
    val guild: Guild? get() = parent.guild
    val manager: TicketManager get() = parent.ticketManager
    
    val memberIds: List<Long> = json.getLongList("ids.members")
    val members: List<Member>
        get() = guild?.members?.filter { memberIds.contains(it.idLong) } ?: emptyList()
    
    var state: State
        get() = json.getString("state")?.let { State.valueOf(it) } ?: run {
            state = State.OPEN
            State.OPEN
        }
    ...
    var closeData: CloseData?
        get() = json.getJson("data.close")?.let { CloseData(it) }
        set(value) = Unit.also { json["data.close"] = value?.toJson() }
    ...
    fun close(event: ButtonInteractionEvent?, channel: TextChannel, member: Member?, save: Boolean) {
        if (state != State.OPEN) return
        
        state = State.CLOSED
        closeData = CloseData(member)

        val archiveTime = closeData!!.timestamp.plus(category.autoArchiveTime)
        val message = (category.messages.closed
            ?: "${channel.locale.translate("discord.ticket.action.close.info")}\n${channel.locale.translate("discord.ticket.action.archive.info")}")
            .replace("{closedBy}", member?.asMention ?: JDA.selfUser.asMention, true)
            .replace("{member}", this.members.joinToString(" ") { it.asMention }, true)
            .replace("{time}", "<t:${archiveTime.epochSecond}:R>", true)
        ...
        if (save) this.updateConfig()
    }
    
    internal fun archive(channel: TextChannel?, save: Boolean = true) {
        state = State.ARCHIVED
        if (closeData == null)
            closeData = CloseData(null)

        channel?.delete()?.queue()
        if (save) this.updateConfig()
    }
    
    fun isFlag(flag: SupportCategory.CategoryFlag) = category.flags.contains(flag)
    
    private fun updateConfig() {
        manager.updateTicket(this)
    ...
    class CloseData(private val json: JsonManager): Serializable() {
        companion object {
            operator fun invoke(member: Member?): CloseData = CloseData(JsonManager(
        ...
        override fun equals(other: Any?): Boolean = other is CloseData && other.timestamp == timestamp
    }
    enum class ButtonAction(private val consumer: SupportTicket.(event: ButtonInteractionEvent, initiator: Member, channel: TextChannel) -> Unit) {
        MANAGE({ event, initiator, _ ->
            val locale = event.userLocale.toLocale()

            val canCloseMember = !this.category.isFlag(SupportCategory.CategoryFlag.MEMBER_CANT_CLOSE) && this.memberIds.first() == initiator.idLong
            val canCloseRole = initiator.idLong !in this.memberIds && initiator.roles
                ...
                .any { it.isFlag(RoleData.RoleFlag.CLOSE_TICKET) }

            val canClose = initiator.isAdmin || canCloseMember || canCloseRole

            event.replyLocalized("discord.ticket.manage", true).addComponents(ActionRow.of(
                Button.danger(CLOSE.id, locale.translate("discord.ticket.manage.splash.close")).withDisabled(canClose.not()),
            )).queue()
        }),
        CLOSE({ event, initiator, channel ->
            this.close(event, channel, initiator, true)
        }),
