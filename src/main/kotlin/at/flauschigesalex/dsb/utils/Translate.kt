...
fun Locale.translate(key: String, vararg data: Any?): String {
    val parentKey = key.substringBefore(".")
    require(parentKey.isNotEmpty()) { "Parent key must not be empty!" }
    
    val key = key.substringAfter(".")
    require(key.isNotEmpty()) { "Child key must not be empty!" }
    
    return ResourceBundle.getBundle("i18n.$parentKey", this).getString(key).format(*data)
}